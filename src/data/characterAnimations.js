...
// ──────────────────────────────────────────────────────────────
// WALK CYCLES — class-specific gaits
// ──────────────────────────────────────────────────────────────

// Knight walk: heavy, deliberate march. Weight settles on each foot.
// Weapon arm stays closer to the body (carrying a sword).
export const WALK_KNIGHT = {
  duration: 600,
  loop: true,
  ease: 'Sine.inOut',
  keyframes: [
    // Contact: left foot plants, right lifts. Weight shifts left.
    { t: 0.0,  pose: { leftLeg: -0.28, rightLeg: 0.32, armL: 0.20, armR: -0.12, torso: 0.03, head: 0.02, weapon: -0.08 } },
    // Passing: right leg swings through. Torso upright, brief double-support.
    { t: 0.25, pose: { leftLeg: -0.10, rightLeg: 0.05, armL: 0.08, armR: 0.02, torso: 0.01, head: 0.005, weapon: -0.02 } },
    // Contact: right foot plants, left lifts. Weight shifts right.
    { t: 0.5,  pose: { leftLeg: 0.32, rightLeg: -0.28, armL: -0.12, armR: 0.20, torso: -0.03, head: -0.02, weapon: 0.10 } },
    // Passing: left leg swings through.
    { t: 0.75, pose: { leftLeg: 0.05, rightLeg: -0.10, armL: 0.02, armR: 0.08, torso: -0.01, head: -0.005, weapon: 0.03 } },
    // Loop back to contact.
    { t: 1.0,  pose: { leftLeg: -0.28, rightLeg: 0.32, armL: 0.20, armR: -0.12, torso: 0.03, head: 0.02, weapon: -0.08 } },
  ],
};

// Wizard walk: light, floating glide. Minimal leg movement, robe sway.
// Staff stays mostly vertical with a gentle pendulum.
export const WALK_WIZARD = {
  duration: 700,
  loop: true,
  ease: 'Sine.inOut',
  keyframes: [
    { t: 0.0,  pose: { leftLeg: -0.15, rightLeg: 0.18, armL: 0.12, armR: -0.08, torso: 0.015, head: 0.01, weapon: -0.04 } },
    { t: 0.25, pose: { leftLeg: -0.05, rightLeg: 0.03, armL: 0.04, armR: 0.01, torso: 0.005, head: 0, weapon: -0.01 } },
    { t: 0.5,  pose: { leftLeg: 0.18, rightLeg: -0.15, armL: -0.08, armR: 0.12, torso: -0.015, head: -0.01, weapon: 0.05 } },
    { t: 0.75, pose: { leftLeg: 0.03, rightLeg: -0.05, armL: 0.01, armR: 0.04, torso: -0.005, head: 0, weapon: 0.02 } },
    { t: 1.0,  pose: { leftLeg: -0.15, rightLeg: 0.18, armL: 0.12, armR: -0.08, torso: 0.015, head: 0.01, weapon: -0.04 } },
  ],
};

// Bunny walk: bouncy hop. Both legs compress and extend together,
// with a visible up-down rhythm. Arms pump for momentum.
export const WALK_BUNNY = {
  ...
  keyframes: [
    // Crouch (gather)
    { t: 0.0,  pose: { leftLeg: 0.20, rightLeg: 0.20, armL: 0.15, armR: -0.15, torso: 0.06, head: 0.03, weapon: 0.08 } },
    // Launch (extend)
    { t: 0.3,  pose: { leftLeg: -0.25, rightLeg: -0.25, armL: -0.20, armR: 0.20, torso: -0.04, head: -0.03, weapon: -0.10 } },
    // Air (tucked)
    { t: 0.5,  pose: { leftLeg: -0.10, rightLeg: -0.10, armL: 0.05, armR: -0.05, torso: -0.02, head: -0.01, weapon: -0.04 } },
    // Land (absorb)
    { t: 0.7,  pose: { leftLeg: 0.15, rightLeg: 0.15, armL: 0.10, armR: -0.10, torso: 0.04, head: 0.02, weapon: 0.06 } },
    // Settle back to crouch
    { t: 1.0,  pose: { leftLeg: 0.20, rightLeg: 0.20, armL: 0.15, armR: -0.15, torso: 0.06, head: 0.03, weapon: 0.08 } },
  ],
};
...
export const IDLE_KNIGHT = {
  ...
  ease: 'Sine.inOut',
  keyframes: [
    { t: 0.0,  pose: { torso: 0.02, head: 0.01, armL: 0.06, armR: -0.03, leftLeg: -0.04, rightLeg: 0.06, weapon: 0.03 } },
    // Inhale: chest rises, shoulders drop slightly, head lifts
    { t: 0.4,  pose: { torso: -0.01, head: -0.015, armL: 0.08, armR: -0.05, leftLeg: -0.03, rightLeg: 0.05, weapon: 0.05 } },
    // Exhale: settle back, micro weight shift
    { t: 0.7,  pose: { torso: 0.025, head: 0.02, armL: 0.05, armR: -0.02, leftLeg: -0.05, rightLeg: 0.07, weapon: 0.02 } },
    { t: 1.0,  pose: { torso: 0.02, head: 0.01, armL: 0.06, armR: -0.03, leftLeg: -0.04, rightLeg: 0.06, weapon: 0.03 } },
  ],
};

// Wizard idle: contemplative sway, staff pendulums gently,
// free hand gestures subtly (fingers twitching with magic).
export const IDLE_WIZARD = {
  duration: 3200,
  loop: true,
  ease: 'Sine.inOut',
  keyframes: [
    { t: 0.0,  pose: { torso: 0.01, head: 0.02, armL: 0.05, armR: -0.02, weapon: 0.04, leftLeg: 0, rightLeg: 0 } },
    { t: 0.35, pose: { torso: -0.015, head: -0.01, armL: 0.10, armR: -0.06, weapon: -0.03, leftLeg: 0.01, rightLeg: -0.01 } },
    { t: 0.65, pose: { torso: 0.02, head: 0.015, armL: 0.03, armR: 0.02, weapon: 0.06, leftLeg: -0.01, rightLeg: 0.01 } },
    { t: 1.0,  pose: { torso: 0.01, head: 0.02, armL: 0.05, armR: -0.02, weapon: 0.04, leftLeg: 0, rightLeg: 0 } },
  ],
};

// Bunny idle: restless energy — constant micro-hops, ears twitch
// (head bobs), weight shifts side to side, fists pump slightly.
export const IDLE_BUNNY = {
  duration: 1800,
  loop: true,
  ease: 'Sine.inOut',
  keyframes: [
    { t: 0.0,  pose: { torso: 0.02, head: 0, armL: 0.08, armR: -0.08, leftLeg: 0.05, rightLeg: -0.03, weapon: 0.04 } },
    { t: 0.2,  pose: { torso: -0.01, head: -0.03, armL: 0.04, armR: -0.04, leftLeg: -0.02, rightLeg: -0.02, weapon: 0 } },
    { t: 0.5,  pose: { torso: -0.02, head: 0.02, armL: -0.06, armR: 0.10, leftLeg: -0.03, rightLeg: 0.05, weapon: -0.03 } },
    { t: 0.7,  pose: { torso: 0.01, head: 0.03, armL: 0.02, armR: -0.02, leftLeg: -0.01, rightLeg: -0.01, weapon: 0.02 } },
    { t: 1.0,  pose: { torso: 0.02, head: 0, armL: 0.08, armR: -0.08, leftLeg: 0.05, rightLeg: -0.03, weapon: 0.04 } },
  ],
};
