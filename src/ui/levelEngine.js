...
function LV_draw(t) {
  ...
  // Objects
  LV_drawObjects(camX, camY, ts, t);
}

// ── Trapped-hero prison + signature-secret medallion ──────────────
// Both draw routines were lost when the hand-crafted maze art was
// restored (PR #29 rewrote this file from an older base that predated
// the secret system). The data flowed to the engine the whole time —
// heroes carried `prison`/`heroCanvas`, secrets carried `glyph` — but
// with the draw functions gone they rendered as nothing but a shadow.
// Restored here so caged heroes and secret objects are visible again.
...
function LV_drawObjects(camX, camY, ts, t) {
...
// Ported from v2 for hero-rescue / secret-sequence gameplay compatibility.
